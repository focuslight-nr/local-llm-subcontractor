...
verdict: 4件とも不採用。Ornithの優位はアーキテクチャではなくモデル固有と結論
status: current
...
**答えはモデル固有**でした。同じ構成の4件は品質で届かないだけでなく、**速度でも
Ornithの2〜3.6倍**かかっています。アクティブパラメータ数が同じでも、推論の効率と
精度は訓練次第です。「MoEだから速い」は選定基準になりません。
