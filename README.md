...
Ollama 0.32.8 が安定版になったら再評価する価値はあります。なお第2段タスクでの
Muse Glimmerは未計測です(削除後に追加したため)。
...
環境変数: `BENCH_OLLAMA_MODELS`(対象モデル), `BENCH_TASKS`(タスク絞り込み,
例 `BENCH_TASKS=4_mutation,6_csv`), `BENCH_TEMPERATURE`(既定0.2)。
