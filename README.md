...
### 世代交代は自明ではない: Qwen3.8-27B(2026-08-15)

同じ27B級の後継世代。公開当日にOllama公式へ登録されたため全6タスクで比較しました
(要 Ollama 0.32.13 以上。0.32.9 では `pull` が412で弾かれます)。

| タスク | qwen3.6:27b(現行) | qwen3.8:27b(新世代) |
|---|---|---|
| 1 pytest生成 | ✅ 8テスト (52s) | ✅ 14テスト (50s) |
| 2 仕様追従コード | ✅ 7/7 (17s) | ✅ 7/7 (11s) |
| 3 ログ→JSON抽出 | ✅ (35s) | ✅ (48s) |
| 4 ミューテーション | ✅ 3/3検出 (104s) | ✅ 3/3検出 (233s) |
| 5 矛盾仕様の検出 | ✅ 報告 (5s) | ✅ 報告 (7s) |
| 6 CSVパーサ | ✅ 10/10 (61s) | ❌ **3回中3回とも失敗** (34–37s) |
| **合計** | **6/6 / 275秒** | **5/6 / 387秒** |

6番はフレーキーではなく再現する弱点でした(3回実行して 2/10・9/10・2/10)。いずれも
**末尾の空フィールドの扱い**を誤り、常に余分な空文字を足すか、逆に落とすかのどちらかです。
加えて生成速度も 8.7–11.4 → 4.8–8.3 tok/s と低下しました(1〜5番はn=1、6番はn=3)。

**新しい世代が、この用途で必ず優れているとは限りません。** テスト生成の網羅性は向上して
いる一方、細かい仕様の詰めでは旧世代が上回りました。既定は `qwen3.6:27b` のまま据え置きです。
